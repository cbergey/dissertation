...
As an adult-adult speech comparison, we used data from the Conversation Analytic British National Corpus, a corpus of naturalistic, informal conversations XXXCITE. We excluded any conversations with child participants. 
...
Each participant rated 35 adjective-noun pairs, and we aimed for each pair to be rated five times, for a total of 910 rating tasks. Participants were allowed to rate more than one set of pairs. Distribution of pairs was balanced with a Mongo database that tracked how often sets of pairs had been rated. If a participant allowed their task to expire with the task partially complete, we included those ratings. Overall, participants completed 32,461 ratings. After exclusions using an attention check that asked participants to simply choose a specific number on the scale, we retained 32,293 judgments, with each adjective–noun pair retaining at least two judgments.
...
coca_token_estimate <- mean_cabnc_token %>% pull(estimate)
coca_token_statistic <- mean_cabnc_token %>% pull(statistic)
coca_token_p <- mean_cabnc_token %>% pull(p.value) %>% printp()
...
The human typicality ratings were combined with usage data from our corpora to let us determine the extent to which parents use language to describe typical and atypical features. In our analyses, we token-weighted these judgments, giving higher weight to pairs that occurred more frequently in children's inputs. However, results are qualitatively identical and all significant effects remain significant when examined on a type level.
...
halves_data %>%
  ...
  mutate(measure = factor(measure, 
                          levels = c("Wiki word2vec", "LDP word2vec"))) %>%
